...
if (isset($_POST['btn-signup'])) {
    ...
    if (!$error) {
        ...
        $query = "INSERT INTO users(first_name, last_name, password, phone_number , email, address, picture)
                  VALUES('$fname', '$lname', '$password', $phone_number, '$email', 'address', '$picture->fileName')";
...
<body>
    <div class="container">
        <form style="margin-left: 10rem; margin-top:8rem;" class="w-75 shadow-lg p-3 mb-5 bg-body rounded" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off" enctype="multipart/form-data">
            <h2>Sign Up.</h2>
            <hr />
            <?php
            if (isset($errMSG)) {
            ...
            <div class="d-flex">
                <input class='form-control w-50' placeholder="Phone Number"  name="phone_number" value="<?php echo $phone_number ?>" />
                <span class="text-danger"> <?php echo $dateError; ?> </span>

                <input class='form-control w-50' type="file" name="picture">
                <span class="text-danger"> <?php echo $picError; ?> </span>
            </div>
            <input name="address" class="form-control" placeholder="Address" maxlength="40" value="<?php echo $address ?>" />
